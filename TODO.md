...
- [ ] 节点执行引擎的流式输出与取消逻辑完善。
- [ ] 移动端/小屏适配。
- [x] 单元测试地基（Vitest，jsdom 环境）已立：覆盖 topoSort / expr / rateLimiter / nodeCache / subgraph / pipeline / executor(纯逻辑) / graphAlgo(纯图算法内核) 共 135 项单测，`npm run test` 可回归（2026-08-07）。`npm run headless examples/headless-demo.json` 已接入 GitHub Actions CI（tsc + 单测 + 纯本地工作流冒烟），并修复了 headless 纯 Node 运行的两处缺陷（i18n 的 import.meta.glob 降级、tsconfig 别名）。executor 的 `runWorkflow` 调度内核已开始拆分为纯函数（第一步：下游 BFS / 可达性 / 循环体 / scope 串行化并查集；第二步：增量/全量执行集 `computeExecutionSet` + 分支剪枝判定 `isBranchPruned` 已从 `executor.ts` 抽至 `src/engine/graphAlgo.ts` 并补 14 项单测，行为零回归，headless 冒烟一致）。
