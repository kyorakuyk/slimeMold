...
import { projectWorkerRecoveryActions, type WorkerRecoveryAction } from './workerRecoveryCapabilityProjection';
...
export interface WorkerRunView {
  ...
  recoveryActions: readonly WorkerRecoveryAction[];
...
export function workerRunViewsFor(
  ...
  return runs
    ...
    .map((run) => {
      const recovery = recoveries.find((item) => item.runId === run.runId);
      const recoveryActions = recovery
        ? projectWorkerRecoveryActions({
          run,
          recovery,
          taskGraph: taskGraphs.find((graph) => graph.id === run.taskGraphId),
