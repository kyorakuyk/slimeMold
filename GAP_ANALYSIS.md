...
| 结果缓存 | 子图缓存命中跳过重复计算 | 无，每次重算全部节点 | 中 | 高 | |
| 增量执行 | 只重跑脏节点及其下游 | 仅全局重跑 + 单节点 `retryNode` | 中 | 高 | |
| 节点生态 | 数百种官方 + 社区节点 | 9 个内置节点 | 中 | 中 | |
| 端口类型校验 | 端口声明类型，连线前校验兼容 | 端口无类型，任意连接，运行时才报错 | 低 | 低 | |
| 中断粒度 | 单节点 cancel + 重算子图 | 仅全局 `AbortController` | 中 | 中 | |
| API 模式 | 可纯 API 提交 prompt（headless） | 仅 UI 触发运行 | 低 | 低 | |
| 观察/调试 | 节点级图像预览 + 实时进度条 | 日志 + 节点状态色 + 运行历史 | 低 | 低 | |
| 工作流分享 | `workflow.json` + API 调用 + 复制子图 | `ProjectFile` 持久化 + 单节点重试 | 低 | 低 | |
| 分支/流程控制 | switch / 条件节点丰富 | 仅线性 DAG，无分支节点 | 中 | 中 | |
