...
- [x] **测试体系（地基已立，2026-08-07）**：新增 `vitest.config.ts`(jsdom 环境) + `vitest.setup.ts`(polyfill matchMedia/ResizeObserver/structuredClone) + `@types/node`/`jsdom` devDependency + `package.json` 的 `test`/`test:watch` 脚本。已覆盖纯/近纯模块单测 **121 项全通过**：`topoSort`、`expr`、`rateLimiter`、`nodeCache`、`subgraph`(展开/端口推断/跨边界连线重定向/循环引用)、`pipeline`(定义/黑板/advance 正向传播/rework 回流)、`executor`(collectInputs 汇集/resolveCapability 分级/applyCapability 裁剪/getActiveRunId 代次)、`graphAlgo`(从 executor 拆出的纯图算法内核：下游 BFS/可达性/循环体/scope 串行化并查集)。`npm run test` 可回归运行。
