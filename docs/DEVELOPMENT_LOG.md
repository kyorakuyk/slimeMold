...
- canonicalizer 对 object key、task/effect ordering、evidence references、Windows path representation和 optional fields做纯内存规范化；exported canonicalizer 同时校验 versioned DTO 的 required/allowed keys、status/number/reference invariants；fingerprint 输出 `worker-recovery-facts-v1:sha256:<hex>`，仅使用 Web Crypto SHA-256，不接 ProjectFile、eventBuffer、side-effect journal、controller或native host。
