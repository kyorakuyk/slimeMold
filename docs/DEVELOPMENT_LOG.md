...
### 7.313 unverified：extract Worker recovery audit controller
...
- 验证：recovery-focused `7 files / 27 tests`；完整 Node `165 test files / 1252 tests`；build通过（最大 chunk `1,183.43 kB`，保留既有 dynamic/static import 与大 chunk warnings；测试保留既有 GUI probe stderr）；i18n `1026/1026`；tsc、diff check通过。当前标记 `unverified`，等待 exact Worker recovery audit controller reviewer。
