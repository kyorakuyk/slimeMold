...
### 7.311 unverified：extract queued Worker host infrastructure wiring
...
- 验证：focused `10 files / 73 tests`；完整 Node `163 test files / 1244 tests`；build通过（最大 chunk `1,181.87 kB`，保留既有 dynamic/static import 与大 chunk warnings；测试保留既有 GUI probe stderr）；i18n `1026/1026`；tsc、diff check通过。当前标记 `unverified`，等待 exact queued Worker host infrastructure reviewer。
