...
### 1.4 与 Static Graph Module 的关系

插件节点是 Graph Module 的一种实现和分发包装，不等同于 Static Graph 本身：

```text
Graph Module → 可复用能力的语义定义、输入输出和边界契约
Plugin       → 模块的安装、分发、manifest、实现后端和隔离策略
```

Static Graph 也可以由内置工作流、外部 Runtime 或普通代码实现；插件只负责提供其中一种可安装的执行后端。无论模块由何种后端实现，父图都应通过稳定版本引用，暴露能力、权限、副作用和验收边界，并保留按权限下钻的溯源入口。插件沙箱解决的是“模块如何安全运行”，不是“模块如何成为项目事实源”。
