...
- `src/engine/graphAlgo.test.ts` —— 执行引擎「纯图算法内核」（从 `executor.ts` 拆出）：`computeDownstream` 下游 BFS 收集、`isReachable` 可达性检测（loopGate 回环）、`collectReachable` 循环体求解、`scopesOfNode`/`computeScopeClusters` B-full scope 串行化并查集分区、`planClustersPerStage` 每层 scope 簇预计算（层→簇映射）、`computeExecutionSet` 增量/全量执行集计算（dirty/force/虚拟节点）、`isBranchPruned` 分支剪枝判定（含 skipFailed 上游失败不剪枝）、`shouldContinueLoop` 循环迭代决策（loopGate pass 分支激活 / 最大轮数截断）、`resolveNodeExecutionMode` 单节点执行路径判定（上游失败传染/类型缺失/bypass/mute/增量跳过/正常执行，按优先级收敛为 mode）
- `src/engine/runtime.test.ts` —— 执行引擎与状态存储的「解耦接缝」`ExecutionRuntime` 接口 + `createStoreRuntime` 默认实现：验证日志/进度/历史/成本/节点状态/资产/边七类输出动作正确转发到 store（mock store 契约测试）；`runWorkflow` 与 `executeNode` 内的全部「只写」输出已收口经该接缝，使 executor 调度内核不再直接触碰 store 写路径
- `src/nodes/builtinHelpers.test.ts` —— `builtin.ts` 抽离出的「与 store 无关的纯函数」子模块：模板渲染 `renderTemplate`、文件名推断 `inferFilename`/`inferAssetKind`/`langToExt`、真值判断 `isTruthy`、JSON Schema 校验 `checkJsonSchema`、架构/计划文本解析 `extractModulesFromDesign`/`extractTasksFromPlan`/`normalizeCategory`
- `src/store/workflowSerialize.test.ts` —— `workflowStore.ts` 抽离出的「纯序列化/转换」子模块：画布态⇄磁盘态拍平还原 `toDisk`/`fromDisk`/`flowNodesFrom`/`flowEdgesFrom`/`storedNodeOf`/`storedEdgeOf`、运行态清洗 `sanitizeNodes`、工作流/项目组装 `serializeCurrent`/`buildProjectFile`（含多工作流拍平往返等价性）
...
2. `npm run test` —— Vitest 单测（195 项）
