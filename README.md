...
- `src/engine/graphAlgo.test.ts` —— 执行引擎「纯图算法内核」（从 `executor.ts` 拆出）：`computeDownstream` 下游 BFS 收集、`isReachable` 可达性检测（loopGate 回环）、`collectReachable` 循环体求解、`scopesOfNode`/`computeScopeClusters` B-full scope 串行化并查集分区、`computeExecutionSet` 增量/全量执行集计算（dirty/force/虚拟节点）、`isBranchPruned` 分支剪枝判定（含 skipFailed 上游失败不剪枝）、`shouldContinueLoop` 循环迭代决策（loopGate pass 分支激活 / 最大轮数截断）
...
2. `npm run test` —— Vitest 单测（142 项）
