...
- `src/engine/runtime.test.ts` —— 执行引擎与状态存储的「解耦接缝」`ExecutionRuntime` 接口 + `createStoreRuntime` 默认实现：验证日志/进度/历史/成本五类输出动作正确转发到 store（mock store 契约测试）
...
2. `npm run test` —— Vitest 单测（162 项）
